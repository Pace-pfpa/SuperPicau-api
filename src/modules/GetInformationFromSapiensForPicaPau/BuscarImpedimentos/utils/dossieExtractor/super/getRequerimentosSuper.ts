...
export async function getRequerimentosSuper(dossie: JSDOMType): Promise<IRequerimentos[]> {
    const arrayRequerimentos: IRequerimentos[] = [];
    const possuiRequerimentos = await hasRequerimentos(dossie, 6);
    
    if (!possuiRequerimentos) return arrayRequerimentos;

    try {
        const baseXPath = "/html/body/div/div[6]/table/tbody/tr";
        ...
        console.error("Error in getRequerimentosSuper:", error.message);
        return [];
    }
}
